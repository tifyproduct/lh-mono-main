...
import { encodeFilterParams, decodeFilterParams } from '$lib/utils/modifyUrlString';
...
export const load: PageServerLoad = async ({ params, fetch, url, locals }) => {
	...
	// TODO: could be improved, due to collection query already called in client side
	...
	const decodedFilterParams = filterParams ? decodeFilterParams(filterParams) : '';
	let reverse: boolean;
	...
	if (decodedFilterParams && decodedFilterParams.length > 0) {
		for (const filterParam of decodedFilterParams.split(';')) {
	...
	const subBrands = collection?.subBrands?.references?.nodes.map((data) => {
		return {
			title: data?.title,
			handle: data?.handle,
	...
	return {
		productPagination,
		paginationInfo: collection?.products?.pageInfo,
		activeSort: sortKey,
		sortReverse: reverse,
		...
		seo: {
			...
			metaDescription:
				params.store === 'sg'
					? collection.seoSgDesc
						? collection.seoSgDesc.value
