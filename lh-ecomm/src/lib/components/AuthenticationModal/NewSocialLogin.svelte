<script lang="ts">
	...
	const socialLogin = async () => {
		...
		try {
			...
			// If user does not exist in Internal DB
			if (!resultChecking) {
				...
				// User Exist in Internal DB and Login in DB
				...
				// If user exist but token not saved in Internal DB
				if (result.success && !result.token) {
					// Ask token from Shopify and saved the token in Internal
					const responseLogin = await fetch('/api/auths/login-shopify', {
						method: 'POST',
						headers: {
							'Content-Type': 'application/json'
			...
			console.error(`Social Login error for ${email}: `, err);
			errorMessage = err instanceof Error ? err.message : 'Internal Server Error, Try again later.';
...
{#if showGoogle}
	<Button
		class="inline-flex items-center justify-center w-12 h-12 border-[1px] border-black-2 rounded-full"
