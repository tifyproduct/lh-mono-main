import type { Request } from "@vercel/functions/headers";
...
export async function googleIpChecker(request: Request) {
    const clientIp = request.headers.get('x-forwarded-for')?.split(',')[0] || request.headers.get('remote-addr');
 
    let resultIpChecking = false;
    
    googleIps.forEach((ip) => {
        const isGoogleIp = ipRangeCheck(clientIp || '', ip.ipv4Prefix);

        if(isGoogleIp) resultIpChecking = true;
    });

    return resultIpChecking;
}